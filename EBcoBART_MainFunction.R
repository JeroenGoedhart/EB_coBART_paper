...
EBcoBART <- function(Y,X,CoData, model,
                     ...
                     Info = T, Seed = T,
                     ndpost = 10000, # number of posterior samples after burn-in period
                     nskip = 10000, # number of burn-in samples
                     nchain = 10, # number of independent mcmc chains
  ...
  for (i in 1:nIter) {
    ...
    print(c(probs[1],probs[2],probs[140]))
    ...
    if (EB == T) {
      trees <- extract(fit,"trees",chainNum = c(1:nchain), sampleNum=c(sample(1:ndpost,0.25*ndpost,replace = F))) # tree structures, for computation, we only select a part of the tree
